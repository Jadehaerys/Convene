...
export default function Dashboard({ role = 'student', onLogout, onChatSend }) {
  ...
  const buildFallbackReply = (message) => {
    ...
    return 'Got it. I am backend-ready — connect `onChatSend` to your API and I will respond with live data.';
  };
  ...
  const handleChatSubmit = async (event) => {
    ...
    try {
      ...
      if (typeof onChatSend === 'function') {
        const response = await onChatSend(trimmedMessage, {
          role,
          activeStudentView,
        ...
        if (typeof response === 'string') {
          ...
          assistantText = response.message || response.reply || response.text;
        }
      }
        ...
        {isStudent && activeStudentView === 'find-tutors' && (
          <div className="dash-coming">
            ...
            <p className="dash-coming__sub">
              Browse verified tutors by subject, availability, teaching style, and on-chain ratings. Your personalized tutor list will appear here.
            </p>
          </div>
        )}
